...
```sh
$ git clone -b puc https://github.com/GeOsmFamily/geosm_services_mvp.git backend
$ cd backend
$ cp .env.example .env
```
...
```
$ docker-compose up -d
$ docker exec -it geoportail_puc bash
$ composer install
```
...
```
$ docker exec -it geoportail_puc bash
$ ogr2ogr -f "PostgreSQL" PG:"host=geoportail_puc_pgsql user=postgres dbname=puc password=postgres" shp/puc.shp   -nln temp_table -nlt MULTIPOLYGON  -lco GEOMETRY_NAME=geom -lco precision=NO
```
